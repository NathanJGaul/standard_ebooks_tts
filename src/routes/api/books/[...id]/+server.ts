...
function parseBookDetailsHtml(html: string, id: string): BookDetails {
  try {
    ...
    const wordCount = parseInt(
      $(`meta[property='schema:wordCount']`, `#reading-ease`).attr(
        `content`,
      ) ?? "",
    ) || undefined;
...
function extractChapters(html: string) {
  ...
  try {
    ...
    const tocSection = html.match(
      /<section id="toc"[^>]*>([\s\S]*?)<\/section>/,
    );

    if (tocSection && tocSection[1]) {
      // Extract links from the TOC
      const chapterMatches = tocSection[1].matchAll(
        /<a href="([^"]+)"[^>]*>([^<]+)<\/a>/g,
      );
        ...
        if (match[1] && match[2]) {
          chapters.push({
            url: match[1].startsWith("http")
              ? match[1]
              : `${STANDARD_EBOOKS_BASE_URL}${match[1]}`,
            title: match[2].trim(),
          });
        }
      }
